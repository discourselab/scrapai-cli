...
def test_own_external_split_and_found_on(proj, capsys):
    ...
    rows = [
        ...
        _row("https://site25.org/cite.pdf", found_on="https://x.org/article-7"),
        _row(
            "https://site25.org/cite.pdf", found_on="https://x.org/article-9"
        ),  # 2nd occurrence
    ...
    assert hosts["site25.org"]["count"] == 2  # citation frequency
    assert hosts["site25.org"]["found_on"].startswith("https://x.org/article")
