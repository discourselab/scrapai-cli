...
def test_dupe_command():
    assert (
        dupe_command("news", "acme_org")
        == "./scrapai dedupe --project news --only acme_org"
    )


def test_bulk_dedupe_command():
    assert (
        bulk_dedupe_command("news", ["a", "b", "c"])
        == "./scrapai dedupe --project news --only a b c"
    )
            ...
            "ai_evidence": [
                {
                    ...
                    "url": "https://acme.org/.well-known/tdmrep.json",
                }
            ],
            "clauses": [
                (
                    "Anti-scraping clause",
                    "https://acme.org/terms",
...
def test_select_and_bulk_command():
    ...
    assert "./scrapai dedupe --project news --only dupey_org" in html
...
def test_coverage_detail_flag_targeted():
    ...
    d = _coverage_detail("news", r)  # the ROW expand, in isolation
    visible = re.sub(
...
def test_compliance_robots_llms_columns():
    ...
    assert (
        '"https://acme.org/robots.txt" target="_blank" rel="noopener">acme.org<'
        not in html
    )
...
def test_overview_parse_date_mixed_tz_comparable():
    ...
    (found live on the proj project)."""
