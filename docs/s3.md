...
**S3 upload runs automatically on production crawls:**
...
- ✅ Deletes the local crawl file after a successful upload (S3 becomes the copy of record)
- ✅ Gracefully skips if boto3 not installed

**Upload Process:**
1. Spider completes crawl successfully (exit code 0)
2. Crawl results saved to `DATA_DIR/<project>/<spider>/crawls/crawl_DDMMYYYY.jsonl`
3. If S3 configured and boto3 installed:
   - File compressed to `.jsonl.gz` (typically 70-90% smaller)
   - Uploaded to S3 bucket
   - Temporary `.gz` file removed, and the local original is deleted after a successful upload
4. S3 path: `s3://<bucket>/<project>/<spider>/crawls/crawl_DDMMYYYY.jsonl.gz`

**Local Files:**
On a successful upload the local crawl file is deleted, so S3 holds the only copy — make sure your bucket is durable and backed up. If the upload fails, the file is kept locally.

## Configuration Check

If uploads are not happening, check that all 4 environment variables are set in `.env`:
- `S3_ACCESS_KEY`
- `S3_SECRET_KEY`
- `S3_ENDPOINT`
- `S3_BUCKET`
...
2. Browse S3 bucket with web console or CLI
3. Check S3 path: `s3://<bucket>/<project>/<spider>/crawls/crawl_*.jsonl.gz`
...
- [Queue Management](queue.md) - Batch processing
