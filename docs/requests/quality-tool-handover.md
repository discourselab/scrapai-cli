...
five real projects, 42 output files
...
9. **tz-aware vs naive date crash** — `overview` crashed on corpora mixing
   timestamp forms (hit live on proj). *overview.py `_parse_date`*
...
- Stale artefacts from the OLD code remain on disk for projects outside the gate
  set (e.g. `data/kb_news_archive/_audit/dashboard_*.html` still shows the
  licence-cell corruption) — regenerate with `./scrapai audit --project <p>`.
- A stray `/tmp/data/news/` tree exists from the pre-fix foreign-cwd smoke test —
