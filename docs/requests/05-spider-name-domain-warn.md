...
- **Status:** RE-APPLIED IN THIS INSTANCE on upstream's rewritten `import_spider()` (2026-07-09). Proven REQUIRED during the news_batch_0 migration: upstream's hard block rejected the legitimate secondary spider `site30_in_pdfindex`; with the warning it imports cleanly. Any fleet with more than one spider per domain needs this.
...
- `site28_gov` + `site28_gov_gc` (General Counsel section of the same site),
- a subdomain grouped under the parent, e.g. `data.site28.gov` → `site28_gov_data`.
