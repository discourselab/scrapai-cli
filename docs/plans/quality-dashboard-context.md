...
1. **No MD/CSV drift.** Byte-identical to the untouched root scripts is the acceptance test:
   `./scrapai audit --project news --no-fetch --no-compliance`, then diff `crawl_audit.csv`,
   `audit_news.md`, `compliance_news.md`, `external_pdf_report.md`. (Verified passing.)
...
- Large PDF/coverage tables → big files (`dashboard_kb_news_archive.html` ≈ 2.6 MB). Fine to open,
...
`.venv/bin/pytest tests/unit/` (currently green); rebuild `dashboard_news.html`; re-run the
