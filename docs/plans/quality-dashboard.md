...
│ ▸ ☐ acme_org    ● ok     ▐████████▌100%  ▐███████▍99%   —       │
│ ▾ ☑ site17_gov    ● broken ▐█▏      12%    ▐█▏     12%    thin?ⓘ  │
│      240 scraped · 1998 rows · 3 files · median 0.4k · Fix:… │
│ ▸ ☐ site25_org    ◔ review ▐███████▏88%    ▐███████▉99%   found→ⓘ│
├───────────────────────────────────────────────────────────────┤
│ 2 selected → ./scrapai dedupe --project news --only … [copy]  │
└───────────────────────────────────────────────────────────────┘
...
1. **No-drift (must pass):** save `audit_<p>.md`, `compliance_<p>.md`,
   `external_pdf_report.md`, `crawl_audit.csv` for `news`; run
   `./scrapai audit --project news --no-fetch --no-compliance`; `diff` → all four
...
3. Build `dashboard_news.html`; confirm three tabs; status facets filter the one table; header
