...
3. **`cli/dedupe.py` — separate command** (mutating; explicit). Calls
   `core.quality.dedupe.run()`. → `./scrapai dedupe --project news`.
...
- The 8 "proj" helper scripts (not yours; a project-specific inference pipeline).
...
1. **No-drift check (most important):** run the untouched root `python3 crawl_audit.py
   --project news` and the new `./scrapai audit --project news --no-compliance --no-html`
...
2. `./scrapai audit --project news` (and `news_archive`) → runs, prints the summary,
