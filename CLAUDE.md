...
- **`crawl-status [--project <p>]`** — one row per spider joining Pueue run-state (running/queued/done/killed/failed) + start/end times with crawl-file counts: items **downloaded**, how many have **content** text (`with-content`; extraction worked), and **`last-item`** (time since the last item was written — a running crawl with a large `last-item` is stalled). The agent-friendly monitor — non-blocking.
