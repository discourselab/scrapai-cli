...
`browser start` keeps one warm browser; `inspect` **and production crawls** auto-route CF verification through it (one browser for the whole machine — N crawls share it instead of one browser each, Cloudflare solved once per host; if the service is unreachable the request fails and Scrapy retries — a crawl never spawns its own browser). `--pool N` caps how many sites can verify **simultaneously** (default 5) — a concurrency knob, not a cache. **Browser crawls are capped at 3 concurrent** in their own Pueue group (`scrapai-browser`; tune with `pueue parallel N --group scrapai-browser`), because 8+ simultaneous CF verifications wedge the service. So raising `--pool` far above the default buys nothing for crawls: the group cap binds first. Size it for `inspect`/screenshot work, ~5. `browser status` reports up/down; `browser restart` bounces it keeping its previous settings (flags override); `browser stop` tears it down. Auto-restarts (same settings) if a crawl finds it dead; a busy service slow to answer is left alone, never replaced. Manage the lifecycle yourself:
