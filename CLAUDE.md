...
- `audit --project <name>` — **read-only.** Per-spider coverage + extraction scoring (`crawl_audit.csv` / `audit_<project>.md`), compliance lenses, and a self-contained HTML dashboard `_audit/dashboard_<project>.html` (tabs: Coverage · Compliance). Classifies every spider into status groups (`ok` / `manual review` / `too few pages` / `incomplete` / `extraction broken`). Uses crawl-recorded stats (`_audit/crawl_stats/`) when present; falls back to fetching sitemaps. Flags: `--no-compliance`, `--no-fetch` / `--fetch-all`, `--only <spider>` (recompute just those; other spiders' rows carry over so the report stays project-wide), `--no-html`, `--refresh` / `--reset`.
