...
`browser start` keeps one warm browser; `inspect` **and production crawls** auto-route CF verification through it (one browser for the whole machine — N crawls share it instead of one browser each, Cloudflare solved once per host). `--pool N` caps concurrent sites (default 5). `browser status` reports up/down; `browser stop` tears it down. Cold-starts on its own if not running, but a per-call cold start re-solves Cloudflare every time — so manage the lifecycle yourself:
...
- **`crawl-status [--project <p>]`** — one row per spider joining Pueue run-state (running/queued/done/killed/failed) + start/end times with crawl-file counts: items **downloaded**, how many have **content** text (`with-content` %; **PDF/links-only items are excluded** since they have no content by design, so the % isn't misleading), and **`last-item`** (time since the last item was written — a running crawl with a large `last-item` is stalled). The agent-friendly monitor — non-blocking. **Run this before any `browser stop`** to confirm no crawls are live (§6.3).
...
- **Generated URLs:** for enumerable archives with NO followable links (JS-postback nav, sequential IDs, date×page sweeps) → `GENERATED_URLS`: a `{name}`-templated URL + a `vars` map, expanded lazily into the cartesian product at crawl start (no browser, plain HTTP). Each var is `range`/`date`/`list`. Routes each URL to an explicit `callback` (no link-following). `start_urls` may be `[]` when this seeds the crawl. Use only when URLs are predictable; otherwise follow links / sitemap / `PAGINATED_LISTINGS`. Example: `{"template": "https://site/{date}?p={page}", "vars": {"date": {"type":"date","from":"2021-06-30","to":"2026-06-30"}, "page": {"type":"range","from":1,"to":50}}, "callback": "parse_page"}`.
