...
- **`crawl-status [--project <p>]`** — one row per spider joining Pueue run-state (running/queued/done/killed/failed) with crawl-file counts: items **downloaded** and how many have **non-empty content** (extraction worked). The agent-friendly monitor — non-blocking, reports `downloaded` + `non-empty %` per crawl.
