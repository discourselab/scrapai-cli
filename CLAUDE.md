...
- **`crawl-status [--project <p>]`** — one row per spider joining Pueue run-state (running/queued/done/killed/failed) + start/end times with crawl-file counts: items **downloaded** and how many have **content** text (`with-content`; extraction worked). The agent-friendly monitor — non-blocking, reports `downloaded` + `with-content %` per crawl.
