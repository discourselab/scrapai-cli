...
#   data/<project>/_audit/compliance/<orgslug>/<YYYY-MM-DD>/   (orgslug: acme.org -> acme_org)
...
def _allow_target_url(pat, domain):
    ...
    `^https://rmi\\.org/resources/[^/]+/?$` → `https://acme.org/resources/x`. Strips a leading
...
def _spider_cfg_path(project, domain):
    ...
    domain slug (e.g. data.site28.gov lives in folder `site28_gov_data`). None if not found.
    """
    direct = os.path.join(
...
def spider_dir_for_domain(project, domain):
    ...
    works even when the folder name isn't the domain slug (e.g. site28_gov_data)."""
