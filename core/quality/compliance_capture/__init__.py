...
record of what a site's terms/robots said is preserved over time (terms change):
  data/<project>/_audit/compliance/<orgname>/<YYYY-MM-DD>/   (orgname = domain slug, acme.org -> acme_org)
...
which captures every project domain inline; new domains only, unless a re-capture flag):
  ./scrapai audit --project news                     # capture new domains + build reports
  ./scrapai audit --project news --refresh           # re-capture existing snapshots (keep history)
  ./scrapai audit --project news --reset             # re-capture, overwriting snapshots (no history)
  ./scrapai audit --project news --no-compliance     # skip compliance entirely (cache only)
